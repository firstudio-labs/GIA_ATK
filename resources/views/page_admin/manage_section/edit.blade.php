          ...
          <div class="row align-items-center">
            <div class="col-md-12">
              <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">Form Edit Section</li>
              </ul>
            </div>
            <div class="col-md-12">
              <div class="page-header-title">
                <h2 class="mb-0">Form Edit Section</h2>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="col-sm-10">
          <div class="card shadow-sm">
            <div class="card-header">
              <h5>Form Edit Section</h5>
            </div>
            <div class="card-body">
              <form action="{{ route('manage-section.update', $section->id) }}" method="POST" id="sectionForm">
                @csrf
                @method('PUT')
                <div class="row">
                  <div class="col-md-8">
                    <div class="form-group">
                      ...
                      <select name="name" class="form-control @error('name') is-invalid @enderror" required>
                        ...
                        <option value="Top Produk" {{ old('name', $section->name) == 'Top Produk' ? 'selected' : '' }}>Top Produk</option>
                        <option value="Recent" {{ old('name', $section->name) == 'Recent' ? 'selected' : '' }}>Recent</option>
                        <option value="Best Seller" {{ old('name', $section->name) == 'Best Seller' ? 'selected' : '' }}>Best Seller</option>
                        <option value="Top" {{ old('name', $section->name) == 'Top' ? 'selected' : '' }}>Top</option>
                        <option value="Top Rating" {{ old('name', $section->name) == 'Top Rating' ? 'selected' : '' }}>Top Rating</option>
                        <option value="Best Product" {{ old('name', $section->name) == 'Best Product' ? 'selected' : '' }}>Best Product</option>
                      </select>
                  ...
                  <div class="col-md-2">
                    <div class="form-group">
                      ...
                      <input type="number" name="discount_percentage" class="form-control @error('discount_percentage') is-invalid @enderror" value="{{ old('discount_percentage', $section->discount_percentage) }}" min="0" max="100">
                  ...
                  <div class="col-md-2">
                    <div class="form-group">
                      ...
                      <div class="form-check">
                        <input class="form-check-input" type="checkbox" value="1" id="is_new" name="is_new" {{ old('is_new', $section->is_new) ? 'checked' : '' }}>
                        <label class="form-check-label" for="is_new">Tampilkan badge New</label>
                      </div>
                      @error('is_new')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                    </div>
                  </div>
                </div>
...
<script>
  ...
  const existing = {!! json_encode(is_array($section->product_ids) ? $section->product_ids : []) !!};
  ...
  document.addEventListener('DOMContentLoaded', function(){
    const wrap = document.getElementById('productsContainer');
    if (existing.length) {
      existing.forEach(id => wrap.appendChild(makeProductRow(id)));
    } else {
      wrap.appendChild(makeProductRow());
    }
  });
